...
export async function activate(
  ...
  const workspacePackageProvider = new WorkspacePackageProvider(
    sm,
    publishManager,
    configManager,
  );
  ...
  const statusTreeDisposable = vscode.window.registerTreeDataProvider(
    "verdaccioStatus",
    statusViewProvider,
  );
  const cacheTreeDisposable = vscode.window.registerTreeDataProvider(
    "verdaccioCache",
    cacheViewProvider,
  );
  const analyticsTreeDisposable = vscode.window.registerTreeDataProvider(
    "verdaccioStorageAnalytics",
    storageAnalyticsProvider,
  );
  const workspaceTreeDisposable = vscode.window.registerTreeDataProvider(
    "verdaccioWorkspacePackages",
    workspacePackageProvider,
  );
  const healthTreeDisposable = vscode.window.registerTreeDataProvider(
    "verdaccioRegistryHealth",
  ...
  const profileStatusBarItem = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
  );
  ...
  logManager.log(
    `Extension activated (v${context.extension?.packageJSON?.version ?? "unknown"})`,
  );
      ...
      try {
        ...
        if (err.message && err.message.includes("not installed")) {
          ...
          vscode.window.showErrorMessage(
            `Failed to start Verdaccio: ${err.message}`,
          );
        }
      }
    },
  ...
  const stopCmd = vscode.commands.registerCommand("verdaccio.stop", () => {
    logManager.log("Stopping server...");
    return sm.stop();
  });
  const restartCmd = vscode.commands.registerCommand(
    ...
    async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to restart Verdaccio: ${err.message}`,
        );
      }
    },
  );
  ...
  const openRawConfigCmd = vscode.commands.registerCommand(
  ...
  const openConfigPanelCmd = vscode.commands.registerCommand(
    "verdaccio.openConfigPanel",
    () => {
      ConfigurationPanel.createOrShow(context.extensionUri, configManager, sm);
    },
  );
  const setRegistryCmd = vscode.commands.registerCommand(
  ...
  const resetRegistryCmd = vscode.commands.registerCommand(
    "verdaccio.resetRegistry",
    () => npmrcManager.resetRegistry(),
  );
  const deletePackageCmd = vscode.commands.registerCommand(
  ...
  const addScopedRegistryCmd = vscode.commands.registerCommand(
    "verdaccio.addScopedRegistry",
    async () => {
      const scope = await vscode.window.showInputBox({
        prompt: "Scope name (e.g. @myorg)",
        placeHolder: "@scope",
      });
      if (!scope) {
        return;
  ...
  const editScopedRegistryCmd = vscode.commands.registerCommand(
    "verdaccio.editScopedRegistry",
    async () => {
      const registries = await npmrcManager.listScopedRegistries();
      if (registries.length === 0) {
        vscode.window.showInformationMessage(
          "No scoped registries configured.",
        );
        return;
      }
      const selected = await vscode.window.showQuickPick(
        registries.map((r) => ({ label: r.scope, description: r.registryUrl })),
      ...
      const newUrl = await vscode.window.showInputBox({
        prompt: `New URL for ${selected.label}`,
  ...
  const removeScopedRegistryCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const selected = await vscode.window.showQuickPick(
        registries.map((r) => ({ label: r.scope, description: r.registryUrl })),
        { placeHolder: "Select scope to remove" },
      );
      if (!selected) {
        return;
      }
  ...
  const addAuthTokenCmd = vscode.commands.registerCommand(
  ...
  const rotateAuthTokenCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const selected = await vscode.window.showQuickPick(
        tokens.map((t) => ({
          label: t.registryUrl,
          description: t.maskedToken,
        })),
        { placeHolder: "Select token to rotate" },
      );
      if (!selected) {
        return;
      }
      const newToken = await vscode.window.showInputBox({
        prompt: `New token for ${selected.label}`,
        password: true,
      });
      if (!newToken) {
        return;
  ...
  const removeAuthTokenCmd = vscode.commands.registerCommand(
  ...
  const revealTokenCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const selected = await vscode.window.showQuickPick(
        ...
        { placeHolder: "Select token to reveal" },
      );
      if (!selected) {
        return;
      }
  ...
  const enableOfflineModeCmd = vscode.commands.registerCommand(
  ...
  const disableOfflineModeCmd = vscode.commands.registerCommand(
  ...
  const pruneOldVersionsCmd = vscode.commands.registerCommand(
    "verdaccio.pruneOldVersions",
    async (item) => {
      const packageName = item?.packageName ?? item?.name;
      if (!packageName) {
        const input = await vscode.window.showInputBox({
          prompt: "Package name to prune",
        });
        if (!input) {
          return;
        }
        const keepStr = await vscode.window.showInputBox({
          prompt: "Number of recent versions to keep",
          value: "3",
        });
        if (!keepStr) {
          return;
        }
        await storageAnalyticsProvider.pruneOldVersionsWithConfirmation(
          input,
          parseInt(keepStr, 10),
        );
  ...
  const bulkCleanupCmd = vscode.commands.registerCommand(
  ...
  const publishToVerdaccioCmd = vscode.commands.registerCommand(
  ...
  const promotePackageCmd = vscode.commands.registerCommand(
    ...
    async (item) => {
      ...
      if (!packageName || !version) {
        vscode.window.showWarningMessage(
          "Select a package version to promote.",
        );
        return;
      }
      const targetUrl = await vscode.window.showInputBox({
        prompt: "Target registry URL",
  ...
  const bumpVersionCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      await publishManager.bumpVersion(
        workspaceFolders[0].uri.fsPath,
        bumpType as any,
      );
    },
  );
  const publishAllCmd = vscode.commands.registerCommand(
    "verdaccio.publishAllWorkspacePackages",
    () => workspacePackageProvider.publishAll(),
  );
  const unpublishAllCmd = vscode.commands.registerCommand(
    "verdaccio.unpublishAllWorkspacePackages",
    () => workspacePackageProvider.unpublishAll(),
  );
  const createProfileCmd = vscode.commands.registerCommand(
  ...
  const deleteProfileCmd = vscode.commands.registerCommand(
  ...
  const mirrorDependenciesCmd = vscode.commands.registerCommand(
    "verdaccio.mirrorDependencies",
    () => dependencyMirrorManager.mirrorDependencies(),
  );
  const cacheAllDependenciesCmd = vscode.commands.registerCommand(
    "verdaccio.cacheAllDependencies",
    () => dependencyMirrorManager.mirrorDependencies(),
  );
  const openSettingsCmd = vscode.commands.registerCommand(
    "verdaccio.openSettings",
    () => {
  ...
  const refreshCacheCmd = vscode.commands.registerCommand(
    "verdaccio.refreshCache",
    () => cacheViewProvider.refresh(),
  );
  const refreshAnalyticsCmd = vscode.commands.registerCommand(
    "verdaccio.refreshAnalytics",
    () => storageAnalyticsProvider.refresh(),
  );
  const refreshHealthCmd = vscode.commands.registerCommand(
    "verdaccio.refreshHealth",
  ...
  const stateChangeDisposable = sm.onDidChangeState((state: ServerState) => {
    try {
      logManager.log(
        `Server state: ${state}${state === "running" ? ` (port ${sm.port})` : ""}`,
      );
      logManager.log(`sm.state=${sm.state} sm.port=${sm.port}`);
      updateStatusBar(statusBarItem, state, sm.port);
      statusViewProvider.refresh();
      const autoSet = vscode.workspace
        .getConfiguration("verdaccio")
        .get<boolean>("autoSetRegistry", false);
      if (autoSet) {
        if (state === "running") {
          npmrcManager.setRegistry(`http://localhost:${sm.port ?? 4873}/`);
  ...
  (async () => {
      ...
      if (registerExtension) {
          ...
          actions: [
            {
              ...
              command: "verdaccio.start",
              description: "Start the Verdaccio server",
            },
            {
              label: "$(debug-stop) Stop Server",
              command: "verdaccio.stop",
              description: "Stop the Verdaccio server",
            },
            {
              label: "$(debug-restart) Restart Server",
              command: "verdaccio.restart",
              description: "Restart the Verdaccio server",
            },
            {
              label: "$(output) Show Logs",
              command: "verdaccio.showLogs",
              description: "Show Verdaccio output channel",
            },
            {
              label: "$(edit) Open Config Panel",
              command: "verdaccio.openConfigPanel",
              description: "Open the configuration webview",
            },
            {
              label: "$(file-code) Open Raw Config",
              command: "verdaccio.openRawConfig",
              description: "Open config.yaml in editor",
            },
            {
              label: "$(plug) Set Registry",
              command: "verdaccio.setRegistry",
              description: "Point .npmrc to Verdaccio",
            },
            {
              label: "$(close) Reset Registry",
              command: "verdaccio.resetRegistry",
              description: "Remove Verdaccio from .npmrc",
            },
            {
              label: "$(cloud-download) Mirror Dependencies",
              command: "verdaccio.mirrorDependencies",
              description: "Cache all project deps locally",
            },
            {
              label: "$(package) Publish to Verdaccio",
              command: "verdaccio.publishToVerdaccio",
              description: "Publish current package",
            },
            {
              label: "$(rocket) Publish All Workspace",
              command: "verdaccio.publishAllWorkspacePackages",
              description: "Publish all workspace packages",
            },
            {
              label: "$(globe) Enable Offline Mode",
              command: "verdaccio.enableOfflineMode",
              description: "Serve only from cache",
            },
            {
              label: "$(trash) Bulk Cleanup",
              command: "verdaccio.bulkCleanup",
              description: "Remove stale packages",
            },
            {
              label: "$(key) Add Auth Token",
              command: "verdaccio.addAuthToken",
              description: "Add registry auth token",
            },
            {
              label: "$(list-tree) Add Scoped Registry",
              command: "verdaccio.addScopedRegistry",
              description: "Route a scope to a registry",
            },
            {
              label: "$(bookmark) Create Profile",
              command: "verdaccio.createProfile",
              description: "Save current .npmrc as a profile",
            },
            {
              label: "$(arrow-swap) Switch Profile",
              command: "verdaccio.switchProfile",
              description: "Switch .npmrc profile",
            },
          ],
        });
        ...
        Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("ACS registration timeout")),
              5000,
            ),
          ),
        ]).catch((err) =>
          console.error("[Verdaccio] ACS registration:", err.message),
        );
      }
    } catch (err) {
      console.error("[Verdaccio] ACS registration failed:", err);
    }
    ...
    try {
      ...
      console.error("[Verdaccio] setOutputChannel failed:", err);
    }
    try {
      ...
      console.error("[Verdaccio] diagnosticCommands failed:", err);
    }

    if (mcpServer) {
      const mcpAutoStart = vscode.workspace
        .getConfiguration("verdaccio")
        .get<boolean>("mcp.autoStart", true);
      if (mcpAutoStart) {
        try {
          await mcpServer.start();
        } catch (err) {
          console.error("[Verdaccio] MCP auto-start failed:", err);
        }
      }
    ...
    const autoStartEnabled = vscode.workspace
      .getConfiguration("verdaccio")
    ...
    console.error("[Verdaccio] Optional integrations failed:", err);
  });
    ...
    .then(async (exists) => {
      if (!exists) {
        ...
        if (action === "Generate") {
          await configManager.generateDefaultConfig();
          vscode.window.showInformationMessage(
            "Default Verdaccio configuration generated.",
          );
        }
      }
    })
...
function updateStatusBar(
  ...
  console.log(
    `[Verdaccio] updateStatusBar called: state=${state} port=${port}`,
  );
  switch (state) {
    case "running":
